...
namespace GPRO {
    ...
    protected:
		...
		bool rowWriteFile(const char* outputfile, elemType *data);

    public:
        MetaData* _pMetaData; ///< raster layer metadata
		double dataMin, dataMax, dataMean, dataStdDev, dataRange;
...
readGlobalFileSerial(const char* inputfile) {
	...
	int approxOK = FALSE;
	int force = TRUE;
	CPLErr result = poBandsrc->GetStatistics(approxOK, force, &dataMin, &dataMax, &dataMean, &dataStdDev);
...
template <class elemType>
bool GPRO::RasterLayer<elemType>::
rowWriteFile(const char* outputfile, elemType *data) {
	GDALAllRegister();
	...
	if (_pMetaData->processor_number == 1) {
		poBanddest->RasterIO(GF_Write, 0, 0, _pMetaData->_glbDims.nCols(), _pMetaData->_glbDims.nRows(),
			data, _pMetaData->_glbDims.nCols(), _pMetaData->_glbDims.nRows(),
			_pMetaData->dataType, 0, 0);
	}
	else {
		int nXOff = 0;
...
rowAtOtherLayer(RasterLayer<double>* layer, int row) {
    ...
    return min(scale * row, (double)layer->_pMetaData->_localworkBR.maxIRow());
}
...
colAtOtherLayer(RasterLayer<double>* layer, int col) {
    ...
    return min(scale * col, (double)layer->_pMetaData->_localworkBR.maxICol());
}
