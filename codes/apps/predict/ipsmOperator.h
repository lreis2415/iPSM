 ...
 */
#ifndef IPSMOPERATOR_H_
#define IPSMOPERATOR_H_
...
namespace solim {
class ipsmOperator: public CommonOperator {
public:
    ipsmOperator():
    ...
    explicit ipsmOperator(EnvDataset* envDataset)
        : CommonOperator(envDataset), 
          thred_envsimi(0.5), val_cannot_pred(-1.), 
          Map_Prediction(nullptr), Map_Uncertainty(nullptr) {
    }

    ipsmOperator(EnvDataset* envDataset, vector<EnvUnit *>& sampleEnvUnits)
