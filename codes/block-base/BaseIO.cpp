...
void BaseIO::write(long xStart, long yStart, long numRows, long numCols, float *source, string writeFilename) {
	...
	else isFileInititialized = FALSE;
	if (is3dr) {
		...
		if (!isFileInititialized) {
            ...
            ds=driver->Create(cFileName, xSize, ySize, 1, GDT_Float32, NULL);//fh = GDALCreate(hDriver, fileName.c_str(), xSize, ySize, 1, eBDataType, NULL);
            ...
            ds = (GDALDataset *) GDALOpen(cFileName, GA_Update );//fh = GDALOpen(fileName.c_str(), GA_Update);
            band = ds->GetRasterBand(1);//bandh = GDALGetRasterBand(fh, 1);
		}
